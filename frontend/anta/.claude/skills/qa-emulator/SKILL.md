...
One CLI drives the device: `tool/qa/qa.dart`. Run it from `frontend/anta`.

```powershell
dart run tool/qa/qa.dart <verb> [args]        # works everywhere, ~3 s of startup
dart compile exe tool/qa/qa.dart -o build/qa/qa.exe
build\qa\qa.exe <verb> [args]                  # what you want in a loop
```

`dart run` prints `Running build hooks...` on stderr before every verb — that
is the Dart CLI, not this tool. Compile the exe once per session and the noise
is gone.
...
picks the serial, otherwise the sole attached device, otherwise
`emulator-5554`.
...
| `state [--json]` | `qa state` | `device=emulator-5554  app=pid 6490  resumed=com.alexzamfir.anta/.MainActivity  screen=1280x2856 @480dpi (427 x 952 dp)  run.log=yes  dtd=ws://127.0.0.1:59940/vZ2LA9BdQPg=` |
...
| `run [--fresh] [--seed F] [--define K=V] [--no-qa] [--no-driver]` | `qa run --fresh` | `pid=26952  log=…\build\qa\run.log` / `dtd=ws://…` / `vm=http://…` (about 9 s warm, up to 4 min on a cold build) |
...
| `launch` | `qa launch` | `launched com.alexzamfir.anta/.MainActivity` (no build, uses what is installed) |
| `shot [name] [--scale F] [--full] [--out D]` | `qa shot home` | `D:\…\build\qa\shots\20260913_173744_home.png` |
| `dump [--all] [--json]` | `qa dump` | see below |
| `tap <target> [--nth N]` | `qa tap "Training"` | `tapped 640,1119  #22  View  "Training\n12"  [0,1020][1280,1218]  click,long` |
...
| `errors [--lines N]` | `qa errors` | `no errors in …\build\qa\run.log` |

### Targets

A target is one of:

- `#12` — the flat node index from the last `dump`;
- `123,456` — raw device pixels;
...
- **`uiautomator dump` fails while an animation runs.** `dump` retries three
  times, 300 ms apart. If it still fails, the screen is genuinely still moving.
  Never run two dumps at once: the second crashes with `UiAutomationService …
  already registered!`, which then sits in logcat looking like an app crash.
  `qa errors` scopes its logcat read to the app's own pid for exactly that
  reason.
- **`E/FirebearStorageCryptoHelper: decryption failed`** in `errors` is Firebase
  with no signed-in user, not an app fault. It is the one line you can ignore.

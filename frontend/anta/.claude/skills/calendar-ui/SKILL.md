...
**Adoption rule (owner, 2026-09-26).** New surfaces use the primitives below. An older sheet that still hand-rolls its chrome (a 4 px pill, its own header row, its own height factor) moves to the primitives **when a change opens it and touches that chrome** — never as a drive-by from an unrelated task. Today four sheets speak the language in full (the editor, Repeat, Icon & color, and since 2026-09-26 the detail sheet — the editor's read-only twin, `docs/event-detail-sheet-redesign-roadmap.md`); the Dates sheet uses the rows but hand-rolls its header; the day-list sheet, the overview page and every pre-redesign sheet use none of it.
...
- Height: `FormMetrics.sheetHeightFactor` (0.92) is the editor's fixed height and every sub-sheet's clamp — Repeat, Icon & color and the detail sheet read it (since 2026-09-26). Eighteen older sheets carry eight different factors (0.6–0.92); when one migrates, take the shape above and that constant.

...
linked. Since 2026-09-26 the `calendar-events` skill is the rulebook and this
file is the history: read the section or addendum you need, never the whole file.
...
`getCalendarAppearance()` is one bulk read since 2026-09-01 (it was eleven sequential settings reads before), and resolving it
...
## Addendum (2026-09-26): the skill ledger — narrative moved out of `calendar-events`

On 2026-09-26 the `calendar-events` skill was cut from 14.7k words to a rulebook
(rules, file map, recipes; see `.claude/skills/calendar-events/SKILL.md`). Most of
its narrative paragraphs duplicated the addenda above. The ones below had no
counterpart in this file — some are also told in
`calendar-performance-roadmap-2026-08.md`, `category-scaling-review-fixes.md` and
`COPILOT_CONTEXT.md` — so they are preserved here verbatim, grouped as the skill
had them. Each describes the tree on the day it was written; the rule it carries
is in the skill in one or two lines. Two claims were corrected on the way in: the
editor's scope swap goes through the fork's `loadText` (not `clearHistory()`), and
the general settings page is `SettingsPage`.

### Schema lineage

- Schema lineage (current: **v40**). Latest: **v40** event alerts — `calendar_event_alerts` (up to five per event, one row each: `mode` `notify`/`ring`, **both** offset sets — `offset_minutes` for timed, `days_before` + `day_minute` for all-day, the planner picking by the derived `event.time == null` — plus `sound`, `enabled`, and the five CRDT columns with **no** `DEFAULT ''` on the identity pair, the `calendar_event_skips` rule for a new table), `alert_registrations` (**device-local**: no CRDT block, hard deletes, never exported, never synced — the mirror of what this phone's OS holds, rebuildable by any reconcile, with `day`/`fire_at` as plain epoch-**ms** ints because one is a date-only UTC day and the other a local instant), and `calendar_events.remove_after_alert` (guarded `ALTER TABLE`, no backfill). Additive backup key `eventAlerts` with the standard event-keyed **strand rule** — an absent key alongside a present `calendarEvents` *clears* the table, and here a stranded row does not draw wrong, it rings — plus `removeAfterAlert` riding `calendarEvents`; **no** version bump (7 stays 7), and `alert_registrations` is never in a backup. Three indexes through `DatabaseIndexes.createAlertIndexes`: `idx_calendar_event_alerts_active (event_id) WHERE is_deleted = 0`, `idx_alert_registrations_pending (fire_at) WHERE state = 'pending'`, `idx_alert_registrations_event (event_id)`. See `docs/event-alerts-roadmap.md` → **v39** `notes.label` + `folders.label` (the one optional colour label an item carries; `NOT NULL DEFAULT 0` = `ItemLabel.none`, additive backup key, no index) → **v38** repair step — `_migrateV37ToV38` re-runs the idempotent v37 body and converts/drops a stray `presence_default` column that an intermediate build stamped as 37 on the owner's phone (a correct v37 database passes through untouched; no new column, no backup change) → **v37** assume-absent presence defaults — `calendar_event_absences.status` (`PresenceStatus` `missed`/`present`, `DEFAULT 'missed'`, unknown → missed, never throws) so a live row carries the answer instead of the absence of one, plus `calendar_events.assume_absent` + `assume_absent_from` (nullable date-only UTC, NULL = whole event) and `calendar_event_templates.assume_absent`. Four guarded `ALTER TABLE`s, **no backfill** (every default is what a pre-v37 row already meant), no backup bump, and a **drop-and-recreate** of `idx_calendar_event_absences_active` as `(event_id, day, status) WHERE is_deleted = 0` — the v27 index-swap idiom, because `IF NOT EXISTS` alone strands upgraders on the two-column index and silently costs `getActiveKeys` its covering read; the skip index stayed two columns, so the two delta tables are no longer symmetric. See the hard rule below and the v37 addendum in `docs/calendar-events-feature.md` → **v36** `idx_notes_ltitle` (index-only: the cross-folder wiki-link title lookup; no table, column or backup change) → **v35** `calendar_filter_presets` (saved calendar filters — CRDT-shaped from birth, the whole filter set in one JSON `filters` column, additive backup key `calendarFilterPresets`, no version bump, **no** strand rule; see the hard rule below) → **v34** `calendar_events.show_in_day_rail` → **v33** `calendar_categories.is_hidden` (archive flag — a hidden category leaves `CalendarCategories.visible` but stays in `all` so its events keep their own colour; plain column, no CRDT block; additive backup key `isHidden`, no version bump, absent means visible — see the hard rule below and `docs/category-scaling-roadmap.md`) → **v32** `vocabularies` + `vocabulary_items` (editor autocomplete lists) → **v31** `idx_calendar_event_skips_active` + `idx_calendar_event_absences_active` (partial covering indexes on the delta tables; index-only — no table, column or backup change — created by `DatabaseIndexes.createCalendarDeltaIndexes()`, which is **separate from** `createCalendarIndexes()` because the latter runs inside the v9→v10 migration where neither delta table exists yet) → **v29** `calendar_event_templates` (reusable event presets — CRDT-shaped from birth, additive backup key `eventTemplates`, no version bump, strand rule keyed to *categories*) → **v30** `calendar_event_skips` (cancelled occurrences — additive backup key `eventSkips`, no version bump, standard event-keyed strand rule). Earlier: **v26** `tracks_presence` + `calendar_event_absences`, **v27** `calendar_events` CRDT, **v28** `per_occurrence_descriptions` + occurrence CRDT. **v24** sparse `calendar_event_occurrences` (per-occurrence description overrides — see the hard rules below; additive backup key `eventOccurrences`, **no** version bump, but an absent key alongside a present `calendarEvents` must *clear* the table). v10 calendar tables → v11 `end_date` + time-of-day → v12 `description` → v13 holiday profiles → v14 `note_id` event↔note link → v15 data-driven `calendar_categories` → v16 `color_value`/`tint_icon`/`priority` → v17 `suppressed` holidays → v18 **priority scale inverted** (data-only `p -> 6 - p`; backup version bumped to 7, older backups flip priorities on import) → v19 `retroactive` (additive, default 0 — **no** backup bump; old backups import as forward-only, which is what they were) → v22 **computed holidays** (no column change: deletes seeded built-in rows from `public_holidays`, leaving only custom + suppressed — see the hard rule below) → v23 **yearly counts from zero** (data-only: `count_style` `numbered` → `elapsed` for counting yearly events, repairing v21's flat default) → v20 `count_occurrences` + v21 `count_style` (both additive, defaults 0/`'numbered'`, **no** backup bump — display-only occurrence labels via `RecurrenceRule.elapsedPeriods` + the single `RecurrenceFormatter.countLabel(event, day, l10n)` entry point, never touching `occursOn`: the two styles differ in **exactly one thing, the counting origin**, and the chips say so ("Count from 1" / "Count from 0"): `numbered` = count from 1 ("Day 1", "Week 3" — start day is occurrence one, all sessions of a week share "Week N"); `elapsed` = count from 0 ("0 years", "26 years" — a birth-date anchor yields the age). **Both render on the start day**; both suppress only pre-start retroactive days — keep that symmetry or the chip labels stop being true. The editor default is **frequency-dependent** (`_defaultCountStyleFor`: yearly → `elapsed`, shorter → `numbered`) — counting a yearly event from 1 is off by one against a birthday's reading (born 2000 → 27th occurrence in 2026, but age 26), so never make `numbered` the flat default again; `_countStyleTouched` re-resolves it on frequency change only until the user taps a chip, a saved counting event counts as an explicit choice, and the rows written under the old flat default were repaired by the **v23** data migration (yearly only). The live example renders the first three occurrences, so it must start at 0 for `elapsed` and 1 for `numbered`. the label leads the row subtitle and suffixes the detail sheet's occurrence chips; editor guard mirrors `retroactive`'s — `recurring && _kindSupportsInterval && _countOccurrences` — with style chips + a live three-sample example line; the birthday built-in pre-fills counting with the **elapsed** style alongside the yearly rule). The recurrence **interval** rides inside `rule_payload` JSON with no migration — keep it that way (write only when `> 1`, decode with clamped fallback to `1`; never add an `interval` column).

### Bloc, caches, settings, grid

- `TableCalendar.eventLoader` stays a pure O(1) lookup through `CalendarBloc.eventsForDay` (memoized per-day cache, cap 512). Never dispatch events, call services, or expand recurrences inside the loader. If you add a bloc handler that mutates events or the category filter, call `_invalidateDayCache()` **in the same synchronous turn as the emit, after the last await** — `eventsForDay` reads `state`, so a grid rebuild inside an await between the invalidation and the emit re-warms the cache from the old list and the emit then serves stale days (the create/update fix of 2026-09-25); never invalidate from day/focus/format handlers. **Holidays are the exception no handler can see**: `WorkdaysRecurrence` and `PublicHolidaysOnlyRecurrence` read `PublicHolidays.isHoliday` from inside `occursOn`, and `PublicHolidayService` publishes straight into the static facade, so backup restore and a database switch change membership with nothing dispatched. `PublicHolidays.revision` (bumped in **both** `configure` and `resetCache`, mirroring `EventSkips`) is folded in as a **generation** by `_syncHolidayGeneration()`, called at the top of `eventsForDay` *and* `monthNetFor` — the latter also calls `occursOn`, so it must not rely on the former running first. A generation, never a cache key: the point is to drop the cache, not grow it.

- **The eight calendar services are constructed lazily, not at app start.** `PublicHolidayService`, `CategoryService`, `CalendarEventService`, `EventOccurrenceService`, `EventPresenceService`, `EventSkipService`, `EventTemplateService` and `EventAlertService` (v40) are **not** in `injection.dart` — resolve them with `await X.getInstance()`, never `GetIt.I<>` (GetIt holds the *object*, so after a database switch it hands back one bound to the closed database; `getInstance()` is self-healing). `CalendarBloc._onLoad` resolves all eight through one `Future.wait` — each wrapped so a single failure cannot reject the batch — and **never emits `CalendarPageLoaded` before they resolve**. That invariant is the safety argument for the whole arrangement: the facades are read synchronously from render paths and from `occursOn`, and an unconfigured read is *silent* — cancelled occurrences reappear, every category goes grey, and `PublicHolidays` falls back to fixed dates, which changes **which days a `Workdays` event occurs on**. A new surface that reads any facade must either live under `CalendarPageLoaded` or await the owning service itself. Anything outside the body builder needs its own guard (the FAB is inert while `selectedDay` is null for exactly this reason). `CalendarPageLoading` exists but is **deliberately never emitted** — `Initial` already renders the spinner, and emitting it on a re-load would flash one over a populated calendar.
...
- Date normalization anywhere in calendar code: `DateTime.utc(y, m, d)` to match `CalendarEvent.occursOn`. All occurrence math is O(1) modular arithmetic on date-only UTC — no per-day allocation, no DST-sensitive math. **Hot callers that already hold a normalized day call `occursOnUtcDay` (cached `startDateUtc`/`endDateUtc`, no re-normalization); `occursOn` is the normalizing public wrapper.** `EventSkips.isSkipped` and `EventPresence.isMissed` **require** a date-only UTC day (debug-asserted, allocation-free) — never hand them a raw `DateTime`. **`EventPresence.isMissed(CalendarEvent event, DateTime dayUtc)` is the one entry point for "was this occurrence missed", and since v37 it answers in two steps**: an explicit `calendar_event_absences` row wins outright in *both* directions (a `present` mark beats an assume-absent event, a `missed` mark beats an assume-present one), and only with no mark does it fall through to the event's own default, `CalendarEvent.assumesAbsentOn(day)` = `assumeAbsent && (assumeAbsentFromUtc == null || !day.isBefore(from))`. It takes the **event**, not an id, precisely because of that fallback — all nine read sites already held one. Neither half may read the wall clock: an unconfirmed *future* day of an assume-absent event is missed exactly like a past one, which is what keeps every read path free of a clock and of a midnight rollover. Converting an event **never** rewrites mark rows, flipping the default bumps neither revision (it rides `allEvents` identity), and under `CalendarMissedDisplay.hidden` an unconfirmed assume-absent event vanishes everywhere except the day panel — that exemption is load-bearing, never "clean it up". The `CalendarEvent` constructor is **non-`const`** (the cached UTC fields are `late final`); do not re-add `const`.

### Add button, alerts hub, navigation, l10n

- **The add button is `CalendarAddFab`, and scroll content clears it rather than the button hiding.** `AppSpacing.fabClearance` (88 = 56 button + 16 Scaffold margin + 16 gap) is added to the bottom padding of **every** panel-mode scrollable (agenda / day panel / timeline) — a new panel mode must do the same, or its last row's trailing actions sit under the FAB and cannot be tapped (the 2026-08-23 report). Collapsing on scroll alone is not a fix: it only moves the problem to the moment the user stops. The button is **extended and day-aware** because it targets `selectedDay`, not today — the label (`AgendaListView.shortDayLabel`: Today / Tomorrow / "Aug 23") is the only thing on screen saying where a new event lands; it renders **collapsed and disabled** while `selectedDay` is null. Colour is **per theme** through the pure `calendarAddFabColors`: light is filled (`appearance.accentOr(colorScheme.primary)`, glyph by `ThemeData.estimateBrightnessForColor` — the `CalendarDayCell._onAccent` guard, because a user-picked accent makes `onPrimary` wrong), dark is the container tone (`primaryContainer` pair for the theme accent; a custom accent's own hue: 0.90 glyph over the lightest of 0.32 / 0.28 / 0.24 that clears 4.5:1 — luminous hues like teal step down, purple stays up). Elevation is `CalendarAddFab.elevation` = 2 in every state, never Material's 6. The bottom panel paints `SurfaceRoles.pageGround` and its cards `rowGroup` (both `DaySummaryPanel` and `_AgendaCard`); the app bar pins `backgroundColor: surface` + `scrolledUnderElevation: 0` (the scrolled-under band is a colour state, so the pinned colour is what removes it). The collapse rides a **`ValueNotifier` on `_CalendarViewState`, never `setState`**: a page-level rebuild per scroll tick would dirty the 42-cell grid and the whole panel, defeating `sameGridInputs`/`samePanelInputs`. The page's `NotificationListener<Notification>` delegates to the pure `fabExtendedFor`, which **reads where the content is, never the finger** (2026-09-14): collapse when a `ScrollUpdateNotification` moves content down past `fabTopSlack`, extend when it moves back up or sits at the top, extend on a `ScrollMetricsNotification` at the top (fresh list) and on overscroll past the top, ignore everything else — `UserScrollNotification` is **not** an input (the old `idle → extend` mapping is what made the button flicker and get stuck as a bare `+`). Resets to extended: `selectedDay` change, `didPopNext`, panel expand toggle. It **ignores horizontal scrollables** — the agenda's summary-chip row, the timeline's hour track and the month pager. Long press = `_quickAddFromTemplate` for the same day (manual-trigger `Tooltip` outside a `GestureDetector`, never the FAB's own `tooltip:`). `DevOptions.showAddButtonTrace` captions the button with the last decision.
...
### Agenda, detail sheet, appearance

- **The agenda re-reads the facades on every rebuild (2026-09-26).** A renamed or recoloured category, a holiday profile or a fasting schedule change reaches the panel only as a rebuild — the bloc's reload used to emit a value-equal state that `Equatable` dropped — so `UpcomingAgendaView.didUpdateWidget` runs `_refreshSearchCatalog()` (category revision + locale + the allowlist) and `_annotationsMoved()` (`PublicHolidays.revision`, `FastingCalendar.revision`) and rescans on either, and `_rowsFor`'s memo key carries `CalendarCategories.revision`. The "did you mean" memo is keyed on the rows it accompanies. **The collapse pass honours hidden presence**: `occurrencesInRange(hideMissed:)` skips a missed occurrence in both the tally and the first-seen choice, so a `perEvent` row never vanishes because its first day was missed; the view passes `missedDisplay == hidden`, and that pairing is the one case a presence tick (`occurrenceRevision`) or the display setting rescans for. `UpcomingAgendaFilters._decodeDate` rejects a day the month does not have (Feb 31 no longer rolls into March).
...
### Drill-down rows and the overview page

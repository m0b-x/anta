...
import '../database/daos/event_absence_dao.dart';
...
/// machinery has no counterpart here. What does carry over is the
...
///
/// All CRDT stamping lives in [EventAbsenceDao]; this layer never touches
/// `hlcTimestamp` / `deviceId` / `version` / `isDeleted`, and its cache holds
/// live marks only.
class EventPresenceService {
  static EventPresenceService? _instance;

  late EventAbsenceDao _dao;

  EventPresenceService._();

  static Future<EventPresenceService> getInstance() async {
  ...
  @visibleForTesting
  static Future<EventPresenceService> forTesting(AppDatabase db) async {
    if (_instance != null) return _instance!;
    return _create(db);
  }

  static Future<EventPresenceService> _create(AppDatabase db) async {
    final service = EventPresenceService._();
    service._dao = db.eventAbsenceDao;
    await service._load();
    _instance = service;
  ...
  /// Live marks only, without CRDT identity. Backups are not a sync channel:
  ...
  Future<void> importData(List<dynamic> data) async {
    ...
    final companions = <EventAbsencesCompanion>[];
    for (final raw in data) {
      ...
      try {
        ...
        debugPrint('[EventPresenceService] Import row error: $e');
      }
    }
  ...
  /// Clears every mark. Used when restoring a backup that carried events but
  /// no absence key: the event import wipes and reinserts, so keeping the
  /// previous database's marks would strand them against unrelated event ids.
